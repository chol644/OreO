<template>
  <div class="profile-edit-container d-flex justify-content-center align-items-center bg-light py-5">
    <div class="profile-edit-form p-4 p-md-5 bg-white shadow-sm rounded" style="width: 100%; max-width: 500px;">
      ...
      <form v-if="!isLoading && userData" @submit.prevent="handleUpdateProfile">
        ...
        <div class="d-grid gap-2 mt-4">
          <button type="submit" class="btn btn-update btn-lg fw-bold" :disabled="isSubmitting">
            <span v-if="isSubmitting" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
            {{ isSubmitting ? '수정 중...' : '수 정' }}
          </button>
          <button type="button" class="btn btn-cancel btn-lg fw-bold" @click="cancelEdit" :disabled="isSubmitting">취 소</button>
        </div>
...
const handleUpdateProfile = async () => {
  ...
  try {
    ...
    if (response.status === 200) { // OK status for PATCH/PUT
    ...
    errorMessage.value = "프로필 업데이트 중 오류가 발생했습니다. 다시 시도해주세요.";
  } finally {
    isSubmitting.value = false;
